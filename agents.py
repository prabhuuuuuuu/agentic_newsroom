...
def research_node(state: GraphState):
    ...
    if response.tool_calls:
        for tool_call in response.tool.calls:
            if tool_call['name'] == 'tavily_search_result_json':
...
def critique_node(state: GraphState):
    ...
    message = [SystemMessage(content=system_prompt), HumanMessage(content=f"Review this draft: \n{draft}")
    ]
